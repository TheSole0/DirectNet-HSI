...
class Trainer(object):
    ...
    def train_epoch(self) -> None:
        ...
        # Saving model
        if ((self.epoch + 1) % self.save_freq == 0 or (self.epoch + 1) == self.opt.epochs):
            torch.save(self.model.state_dict(), os.path.join(self.model_path, f'DirectNet_{self.opt.dataset}_{self.epoch+1}.pkl'))
...
def train_model(opt):
    ...
    prefix = 'DirectNet' + '_batch_size_' + str(opt.batch_size) + '_epoch_' + str(opt.epochs)+ '_learning_rate_' + str(opt.learning_rate) + \
...
def main():    
    ...
    parser.add_argument('--dataset', default='Pavia', type=str, help='dataset to use')
